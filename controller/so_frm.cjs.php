...
$('#KdBarang2').combogrid({  
	panelWidth:500,  	
	url: '<?php echo $basedir; ?>models/material/so_grid.php?req=dgDet',  
	idField:'KdBarang2',  
	textField:'KdBarang2',  
	mode:'remote',  
	fitColumns:true,  
...
$('#tl1Sim').click(function(){
	...
	if ($('#so_no').val() == ''){	
		throw "so_no-PO No.";
	} else if ($('#so_date').datebox('getValue') == ''){ 
		throw "so_date-PO Date";
	} else if ($('#currency').val() == ''){	
		throw "currency-Currency";
	} else if ($('#cust').val() == ''){	
		throw "cust-Customer";
	} else if ($('#due_date').datebox('getValue') == ''){ 
		throw "due_date-Due Date";	
	} else if (rows.length == 0){
		throw "so_no-Finished Goods List";	
	} else {
		//FORM LIST BARANG
		nolist_val="";	
		KdBarang2_val="";
		...
		$.post("<?php echo $basedir ?>models/material/so_tuh.php", { 
		aksi: $('#aksi').val(), 
		so_id: $('#so_id').val(),
		so_no: $('#so_no').val(),
