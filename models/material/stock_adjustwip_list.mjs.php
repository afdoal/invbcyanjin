...
function setdg(){
	$('#dg').datagrid({  	
		...
		columns:[[  		
			...
			{field:'action',title:'Action',width:80,
				formatter:function(value,row,index){
					var det = '<a href="#" onclick="window.open(\'stock_adjustwip_pdf.php?NmMenu=Stock Adjustment WIP&opname_id='+row.opname_id+'\', \'_blank\');"><img src="<?php echo $basedir ?>themes/icons/pdf.png"></a>';
					return det;					
				}
			}
		]],
		url: '<?php echo $basedir; ?>models/material/stock_adjustwip_grid.php?req=menu&pilcari='+$("#pilcari").val()+'&txtcari='+$("#txtcari").val(),
		view: detailview,  
		...
		onExpandRow: function(index,row){  
			$('#ddv-'+index).datagrid({  
				url:'<?php echo $basedir; ?>models/material/stock_adjustwip_grid.php?req=listrpt&opname_id='+row.opname_id,  
				fitColumns:true,  
				singleSelect:true,  
				rownumbers:true,
				pagination:true,
