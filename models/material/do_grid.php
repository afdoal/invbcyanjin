...
if ($req=='menu'){
	...
	$q = "SELECT KdBarang AS KdBarang3,KdBarang AS KdBarang2,NmBarang AS NmBarang2,HsNo AS HsNo2,Sat AS Sat2,FORMAT(WPcs, 2) AS weight0,IF(weight>0,FORMAT(weight, 2),0) AS weight,FORMAT(a.qty, 2) AS qty,FORMAT(a.price, 4) AS price,FORMAT(a.qty*a.price, 4) AS amount
		  FROM mat_outdet a 
