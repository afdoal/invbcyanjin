...
$qh = "SELECT matcon_id AS KdBarang,PartNo,NmBarang,HsNo,Sat,Treatment,Ket,cust
	  FROM ppic_fgmatcon a
	  LEFT JOIN mst_barang b ON KdBarang=matcon_id ";
$qh .= "WHERE matcon_id = '$KdBarang0' ";	  
$qh .= "ORDER BY KdBarang ASC";
$runh=$pdo->query($qh);	
...
$html = '<h2 align="center">'.$NmMenu.'</h2>'.		
		...
		<tr>
		  <td width="60"><b>Unit</b></td>
		  <td width="10"><b>:</b></td>
		  <td width="80"><b>'.$rsh[0]['Sat'].'</b></td>
		  <td width="60"><b>Customer</b></td>
		  <td width="10"><b>:</b></td>
		  <td width="200"><b>'.$rsh[0]['cust'].'</b></td>
		  <td width="60"><b>Remarks</b></td>
		  <td width="10"><b>:</b></td>
		  <td width="100"><b>'.$rsh[0]['Ket'].'</b></td>
		</tr>
