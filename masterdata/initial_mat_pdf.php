...
$q = "SELECT wh_name,DATE_FORMAT(date,'%d/%m/%Y') AS date,KdBarang AS KdBarang3,KdBarang AS KdBarang2, NmBarang AS NmBarang2,twhmp,LBar,HsNo AS HsNo2,Sat AS Sat2,FORMAT(weight, 2) AS weight,FORMAT(qty, 2) AS qty
	  FROM mst_barang a 
	  LEFT JOIN mat_stockcard b ON mat_id = KdBarang
	  LEFT JOIN mat_warehouse c ON c.wh_id=b.wh_id  
	  WHERE TpBarang='1' AND b.wh_id='$wh_id' AND date='$date'
	  ORDER BY mat_id ASC";
$run=$pdo->query($q);	
...
$html = '<h2 align="center">'.$NmMenu.'</h2>'.		
		...
		<tr>
		  ...
		  <td width="10"><b>:</b></td>
		  <td width="150"><b>'.$rs[0]['date'].'</b></td>
		</tr>
		<tr>
		  <td width="60"><b>Warehouse</b></td>
		  <td width="10"><b>:</b></td>
		  <td width="150"><b>'.$rs[0]['wh_name'].'</b></td>
		</tr>
